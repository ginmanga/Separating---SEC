...
AA_FINAL_S = AA_FINAL_S.drop(columns=['doc_Date','indfmt','consol','sich'])
...
#drop where pre and post are 0 do not do this now (do later)
...
AA_FINAL_S_FY['post'] = np.where((1993 >= AA_FINAL_S_FY['fyear']) & (AA_FINAL_S_FY['fyear'] >= 1990), 1, 0)
#AA_FINAL_S_FY.drop_duplicates(keep=False,inplace=True)
AA_FINAL_S_FY = AA_FINAL_S_FY.drop_duplicates()
...
###LATER DO it here, take note so I can remake samples later
#temp = temp.drop(temp[temp['sum(pre)'] == 0].index)
#temp = temp.drop(temp[temp['sum(post)'] == 0].index)
###LATER Do this one in bs-data when making the samples
...
temp_nf = temp.copy()
###LATER
indexnames= temp_nf[(temp_nf['sic'] >= 6000) & (temp_nf['sic'] < 7000)].index #put this in a bette rplace
...
sql_test_path = 'C:/Users/Panqiao/Documents/Research/SS - All/MFFS/collapsed_v3.txt'
nf_path = 'C:/Users/Panqiao/Documents/Research/SS - All/MFFS/not_collapsed_v3.txt'
